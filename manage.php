...
<!-- Wrapper -->
	<div id="wrapper">
		...
		<div id="main">
			...
			<!-- Header -->
				<header id="header">
					<a href="#" class="logo"><strong>Manage Vehicle</strong></a>
					<ul class="icons">
				...
				<section>
					...
					<ul class="actions">
						<li><a href="tambahkendaraan.php" class="button primary icon solid fa-plus">Tambah Kendaraan</a></li>
					</ul>
					...
					<div class="posts">
						...
						if (isset($_GET['Merk'])) {
                			...
                			$query = mysqli_query($koneksi,"SELECT * FROM kendaraan ORDER BY id_kend DESC");
						}
						...
						<article>
							<a href="#" class="image"><img src="<?= $data['foto']; ?>" alt="Card image cap" /></a>
							<h3><?= $data['Merk'] ?></h3>
							<p>Harga Sewa: <?= ("Rp. " . $data['harga_sewa'] . "/day"); ?></p>
							<p>NO POLISI: <?= $data['no_polisi']; ?></p>
							<ul class="actions">
								<li><a href="editkend.php?id=<?= $data['id_kend'] ?>" class="button">Edit</a></li>
								<li><a href="hapuskend2.php?id=<?= $data['id_kend'] ?>" class="button primary icon solid fa-trash" onclick="return confirm('Yakin Hapus?')">Hapus</a></li>
							</ul>
						</article>
					<?php 	} ?>
				<?php 	} ?>
